...
| Transport (NI) | `GpibUtils.Visa.Ni` | NI-VISA (default) + native NI-488.2 providers, built against the **official NI/IVI VISA.NET assemblies** from the local NI-VISA install (resolved **by simple name — GAC, then an unversioned disk glob, then an explicit override**; never vendored, never version-pinned; auto-registered by reflection). Where NI-VISA isn't installed it builds an *"NI-VISA unavailable"* stub, so the whole solution still compiles with zero NI setup (and on CI). Pass `-p:RequireNi=true` to hard-fail instead of stubbing. |
