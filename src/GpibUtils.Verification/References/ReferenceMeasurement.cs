...
namespace GpibUtils.Verification.References
{
    ...
    /// uniformly. All adapters run over the shared VISA transport, so a reference is provider-agnostic and a
    /// whole verification is drivable hardware-free against the <c>Simulated</c> provider — the console seeds
    /// the matching simulator models and couples them to the DUT's commanded set-points
    /// (<c>SimulatedHarnessBench</c>). That coupling is exact, so a simulated run exercises the harness but
    /// cannot judge an instrument. See <c>docs/VERIFICATION_HARNESS.md</c>.
    /// </summary>
