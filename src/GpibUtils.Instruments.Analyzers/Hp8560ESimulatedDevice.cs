...
{
    /// <summary>
    /// An in-memory model of an HP 8560E for use with <see cref="SimulatedGpibProvider"/>, rich enough to
    /// drive the <see cref="Hp8560E"/> driver — including the #43 <b>SRQ-edge</b> sweep handshake — end to end
    /// with no hardware. It decodes the setup the driver writes (<c>CF</c>/<c>SP</c>/<c>RB</c>/<c>VB</c>/
    /// <c>ST</c>) and models the 8560-series status byte (Table 7-9): command-complete (0x10) and end-of-sweep
    /// (0x04) are conditions that are TRUE while idle, go false when a sweep starts (<c>TS</c>), and true
    /// again when it finishes; request-service (0x40) latches whenever a masked condition is true.
    ///
    /// <para>Because the SRQ-edge flow must see the sweep go BUSY (the expect bit clear) before accepting the
    /// next request-service as completion, the busy→done transition is advanced per serial poll via
    /// <see cref="SimulatedInstrument.OnSerialPoll"/>: a triggered sweep stays busy for
    /// <see cref="BusyPolls"/> polls and then completes. Set <see cref="SweepCompletes"/> false to model a
    /// sweep that never finishes (the waiter times out); set <see cref="ErrorOnSweep"/> to raise the error
    /// bit at completion.</para>
    /// </summary>
    public sealed class Hp8560ESimulatedDevice
    {
        private const byte EndOfSweep = 0x04, CommandComplete = 0x10, Error = 0x20;
        ...
        public double MarkerFrequencyHz { get; set; } = 1e9;
        ...
        /// <summary>When true, the sweep sets the error bit (0x20) at completion (models an UNCAL/error).</summary>
        public bool ErrorOnSweep { get; set; }

        public Hp8560ESimulatedDevice()
        {
            Instrument = new SimulatedInstrument
            {
                IdentificationString = "HP8560E",
                WriteObserver = Apply,
                Responder = Respond,
        ...
        private void Apply(string command)
        {
            ...
            foreach (var part in raw.Split(';'))
            {
                ...
                if (upper == "IP")
                {
                    _sweep = Sweep.Idle; _mask = 0;
                    _commandComplete = true; _endOfSweep = true; _error = false;
                ...
                if (upper == "TS")
                {
                    ...
                    _commandComplete = false; _endOfSweep = false; _error = false;
                    Recompute(); continue;
                }
        ...
        /// <summary>Advances the busy→done sweep state one serial poll (SRQ-edge handshake).</summary>
        private void AdvanceSweep()
        {
            ...
            _sweep = Sweep.Done;
            _commandComplete = true; _endOfSweep = true; _error = ErrorOnSweep;
            Recompute();
        }

        /// <summary>Publishes the condition bits into the status byte and latches request-service (0x40) when
        /// a masked condition is currently true.</summary>
        private void Recompute()
        {
            byte conditions = (byte)((_commandComplete ? CommandComplete : 0)
                                   | (_endOfSweep ? EndOfSweep : 0)
                                   | (_error ? Error : 0));
            Instrument.StatusByte = conditions;
            Instrument.ServiceRequestPending = (_mask & conditions) != 0;
        }
