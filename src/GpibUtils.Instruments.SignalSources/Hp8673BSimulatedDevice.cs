...
{
    /// <summary>
    /// An in-memory model of an HP 8673B for use with <see cref="SimulatedGpibProvider"/>. It decodes the
    /// HP-IB mnemonics the driver writes (<c>IP</c> / <c>FR … MZ</c> / <c>LE … DM</c> / <c>RF1</c> / <c>RF0</c>)
    ...
    /// </summary>
    public sealed class Hp8673BSimulatedDevice
    {
        ...
        /// <summary>Last commanded frequency in MHz; null after preset / before it is set.</summary>
        public double? FrequencyMHz { get; private set; }

        /// <summary>Last commanded level in dBm; null after preset / before it is set.</summary>
        ...
        public Hp8673BSimulatedDevice()
        {
            Instrument = new SimulatedInstrument
            {
                IdentificationString = "Hewlett-Packard,8673B,0,0",
                WriteObserver = Apply
            };
        }
        ...
        private void Apply(string command)
        {
            ...
            if (upper.StartsWith("FR")) { FrequencyMHz = ExtractNumber(cmd) ?? FrequencyMHz; return; }
            if (upper.StartsWith("LE")) { PowerDbm = ExtractNumber(cmd) ?? PowerDbm; return; }
        }

        // Pulls the first parseable numeric token out of a "FR <val> MZ" / "LE <val> DM" message.
